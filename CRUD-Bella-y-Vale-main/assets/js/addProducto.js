...
async function modalRegistrarProducto() {
	try {
		...
		const myModal = new bootstrap.Modal(
			modalContainer.querySelector("#agregarProductoModal")
		);
		myModal.show();
	} catch (error) {
		console.error(error);
...
async function registrarProducto(event) {
	try {
		...
		if (response.status === 200) {
			// Llamar a la función insertProductoTable para insertar el nuevo registro en la tabla
			window.insertProductoTable();

			setTimeout(() => {
				$("#agregarProductoModal").css("opacity", "");
